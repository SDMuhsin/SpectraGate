...
class MIC(nn.Module):
    ...
    def conv_trans_conv(self, input, conv1d, conv1d_trans, isometric):
        ...
        # isometric convolution
        zeros = torch.zeros((x.shape[0], x.shape[1], x.shape[2] - 1), device=device)
    ...
    def forward(self, src):
        ...
        # merge
        mg = torch.tensor([], device=device)
        for i in range(len(self.conv_kernel)):
            mg = torch.cat((mg, multi[i].unsqueeze(1).to(device)), dim=1)
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # embedding — seasonal_init_dec is [B, seq_len+pred_len, C], so construct
        # matching marks from encoder marks + zeros (x_mark_dec is only label_len+pred_len)
        zeros = torch.zeros([x_dec.shape[0], self.pred_len, x_dec.shape[2]], device=x_enc.device)
        seasonal_init_dec = torch.cat([seasonal_init_enc[:, -self.seq_len:, :], zeros], dim=1)
        mark_zeros = torch.zeros([x_enc.shape[0], self.pred_len, x_mark_enc.shape[2]], device=x_enc.device)
